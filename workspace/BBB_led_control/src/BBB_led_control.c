...
//	sprintf(buf, SYS_FS_LEDS_PATH "/beaglebone:green:usr%d/trigger",led_no);
	sprintf(buf, "/sys/class/leds/beaglebone:green:usr%d/trigger",led_no);
...
//	snprintf(buf,sizeof(buf),SYS_FS_LEDS_PATH "/beaglebone:green:usr%d/brightness",led_no);
	sprintf(buf, "/sys/class/leds/beaglebone:green:usr%d/brightness",led_no);
...
void process_brightness_values(char *value)
{
	if(*value == '1' || *value == '0')
...
//    		int value = atoi(argv[2]);
    		process_brightness_values(argv[2]);
    	}
    	else /* default: */
    	{
